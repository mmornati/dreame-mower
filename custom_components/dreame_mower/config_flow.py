        ...
        if user_input is not None:
            ...
            if username and password and country:
                ...
                try:
                    ...
                    # Mirror async_step_connect's pattern (config_flow.py:218-249):
                    # any unhandled exception during cloud login / device list
                    # parsing is reported to the user as a connection error
                    # rather than letting HA surface it as the unhelpful
                    # 'unknown error'. asyncio.CancelledError subclasses
                    # BaseException in modern Python and is therefore not
                    # caught here, so a config-flow cancel still propagates.
                    errors["base"] = "cannot_connect"
            else:
                errors["base"] = "credentials_incomplete"
